...
# docker hub build command
docker build -f Build.Dockerfile -t dockerdrivemecrazy/bds-web-data:latest-dev .

# docker hub push image
docker push dockerdrivemecrazy/bds-web-data:latests-dev

# docker run local (image from dockerhub) command
docker run -v -d -p 3000:3000 -t dockerdrivemecrazy/bds-web-data:latest-dev