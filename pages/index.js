...
export default function Index(props) {
  ...
  const formatParams = (objParams) => {
    const newObjParams = {}
    Object.entries(objParams).forEach(([key,val]) => {
      const listSplit = key.split('_')
      const checkCal = listSplit.slice(-1)[0]
      if(checkCal === 'to' || checkCal === 'from'){
        const keyObj = listSplit.slice(0, -1).join('_')
        const subKeyObj = checkCal === 'from' ? '$gt' : '$lt'
        const tempObject = newObjParams[keyObj] ? newObjParams[keyObj] : {}
        tempObject[subKeyObj] = Number(val)
        newObjParams[keyObj] = tempObject
      }
      else
        newObjParams[key] = val
    })
    return newObjParams
  }
